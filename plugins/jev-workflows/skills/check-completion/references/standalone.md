...
```sh
node /absolute/path/to/check-completion/scripts/jev.mjs status
node /absolute/path/to/check-completion/scripts/jev.mjs check-completion < request.json
node /absolute/path/to/check-completion/scripts/jev.mjs check-completion --evaluate < request.json
```
...
```json
{
  "claim": "The change passes the required unit tests.",
  "acceptanceCriteria": [
    "The affected test suite was executed on the current source and all assertions passed."
  ],
  "evidence": [
    {
      "id": "tests",
      "text": "The current run reports 5 tests passed and 0 failed.",
      "source": "current test run"
    }
  ]
}
```
